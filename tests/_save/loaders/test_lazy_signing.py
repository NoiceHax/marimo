...
class TestVerificationAndCapabilityValidation(_FileStoreLoaderTest):
    ...
    def test_no_cryptography_on_misses_never_serves(self) -> None:
        """Without cryptography, verify keeps verifying (miss) on a local file
        store rather than serving unsigned bytes — install crypto to use the
        cache. (verification='off' remains the explicit unsigned opt-out.)"""
        from unittest import mock
