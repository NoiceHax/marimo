...
# Settings that an untrusted-origin config layer must not set. Each one takes
# effect before any cell runs, so a layer that sets it decides something the
...
#   runtime.pythonpath   which module an `import` resolves to
#   runtime.dotenv       which files become environment variables
#
# All four are anchored only in trusted user/environment config.
...
def strip_untrusted_config(
    ...
    that repo's author choosing your imports, your environment, or whose signed
    cache you unpickle.

    Set `is_user_layer` for the workspace `.marimo.toml`, which additionally
    loses `cache.store`; see `_UNTRUSTED_USER_LAYER_KEYS`.
    """
    keys = (
