...
!!! warning "Settings that repositories cannot override"
    A `pyproject.toml`, a notebook's script metadata, and a `.marimo.toml` inside
    a project directory all travel with the code, so whoever wrote the repository
    controls them. marimo reads these settings from your own user configuration
    only, and ignores them in all three:

    - `runtime.pythonpath`, which decides where an `import` looks for a module
    - `runtime.dotenv`, which decides what becomes an environment variable
    - `signing` and `cache.verification`, which decide
      [whose cached code you unpickle](../../api/caching.md#cache-signing-and-trust)
...
!!! note "Configuration precedence"
    ...
    **Script config > pyproject.toml config > user config**
    ...
    The ordering covers the settings those layers are allowed to set. The
    settings listed above come from user configuration whatever any other layer
    says.
